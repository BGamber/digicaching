...
let getUserById = async (req, res) => {
  let queryString =
    "SELECT id, email, name, image_url FROM users" +
  ...
  let queryString2 =
    "SELECT u.name, u.image_url FROM users u " +
  ...
  try {
    await Promise.all(
      users.map(
        async user => (user.friends = await db.query(queryString2, [user.id]))
      )
    );
  } catch (err) {
    res.setHeader("Content-Type", "application/json");
  ...
  let queryString3 =
    "SELECT i.id, i.name as item_name, i.description as item_description, " +
  ...
  try {
    await Promise.all(
      users.map(
        async user => (user.inventory = await db.query(queryString3, [user.id]))
      )
    );
  } catch (err) {
    res.setHeader("Content-Type", "application/json");
...
let getItems = async (req, res) => {
  let queryString =
    "SELECT name, description, image_url FROM items" +
    (req.params.id !== undefined ? " WHERE id = $1" : "") +
    ";";
...
let getCollections = async (req, res) => {
  ...
  let queryString2 =
    "SELECT i.id, i.name, i.image_url as item_image_url " +
    "FROM items i JOIN recipes r WHERE r.item_id = $1 AND r.ingredients LIKE '%/' || i.id || '/%';";
  try {
    await Promises.all(
      collections.map(
        async collection =>
          (collection.ingredients = await db.query(queryString2, [
            collection.id
          ]))
      )
    );
...
let getCaches = async (req, res) => {
  ...
  let queryString =
    "SELECT c.id, i.name as item_name, i.description as item_description, " +
  ...
  if (req.params.id) {
    ...
    queryString +=
      "WHERE latitude BETWEEN $4 AND $3 AND longitude BETWEEN $5 AND $6;";
    caches = await db.query(queryString, [
      location[0],
      location[1],
      bounds[0],
...
let claimCache = async (req, res) => {
  let claimedCheck = await db.any(
    "SELECT * FROM claims WHERE cache_id = $1 AND user_id = $2;",
    [req.params.id, req.jwt.userId]
  );
  if (claimedCheck.length > 0) {
    res.status(422).send("Cache Already Claimed By User");
  } else {
    let { latitude, longitude } = req.body;
    let { distancecheck } = await db.one(
      "SELECT (ST_DISTANCE(ST_POINT($1, $2), location) < 50) as distancecheck " +
        "FROM caches WHERE id = $3;",
      [latitude, longitude, req.params.id]
    );
    if (distancecheck) {
      let { item_id } = await db.one(
        "SELECT item_id FROM caches WHERE id = $1",
        [req.params.id]
      );
      ...
      let queryString3 =
        "INSERT INTO claims (user_id, cache_id) " + "VALUES ($1, $2);";
      try {
...
let serverPlaceCache = async () => {
  ...
  let queryString =
    "INSERT INTO caches (item_id, latitude, longitude, location) " +
...
let placeCache = async (req, res) => {
  ...
  if (result !== null && result.quantity > 0) {
    let queryString2 =
      "INSERT INTO caches (item_id, latitude, longitude, location, user_id) " +
      "VALUES ($1, $2, $3, ST_POINT($2, $3), $4) RETURNING id;";
    let newCache;
    try {
      newCache = await db.one(queryString2, [
        item_id,
        latitude,
        longitude,
        req.jwt.userId
    ...
    let queryString4 =
      "UPDATE inventories SET quantity = inventories.quantity - 1 " +
      "WHERE user_id = $1 AND item_id = $2;";
    try {
